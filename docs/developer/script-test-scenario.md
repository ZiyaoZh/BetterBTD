...
页面、视口、元素和状态 ID 必须存在于当前 Game Driver catalog，视口必须归属指定页面。`Element Visible` 只接受已有独立可见性 detector 的元素；仅声明几何边界或动作点不够。
...
协议可以表达 `inLevel.round GreaterThanOrEqual 40`，但当前 Game Driver 只声明了该 value 元素的边界，尚未独立解析生命、金币和回合数。因此当前能力预检会返回 `missingCapabilities: [ElementNumber]` 和退出码 `3`。编排器必须报告 `unsupportedOracle`，不得改用 BetterBTD OCR、脚本进度或日志判定该断言。
