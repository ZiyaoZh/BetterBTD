...
超时后，Runner 按顺序点击配置的 `1920 × 1080` 基准恢复坐标。默认序列为 `(960, 840)`、`(960, 760)`、`(1340, 850)`、`(850, 810)`、`(780, 730)`、`(1140, 730)`、`(80, 55)`。相邻坐标点击之间使用配置的等待时间；全部点击完成后不捕获界面、不检测规范化 UI 状态或画面指纹，直接清除卡死跟踪基线并回到正常导航决策。坐标列表为空时不执行猜测点击，直接失败。
...
- 卡死恢复坐标序列执行完成后直接继续导航；后续仍无进展时会按新的卡死跟踪周期再次恢复。
...
- `Unknown`、`InLevel`、`Loading` 和画面指纹变化下的卡死计时，以及恢复序列完成后继续导航的结果。
