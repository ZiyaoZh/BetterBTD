...
超时后，Runner 按顺序点击配置的 `1920 × 1080` 基准恢复坐标。每次点击后都按配置的等待时间重新捕获界面；一旦界面变化，立即退出恢复并回到正常决策。全部坐标尝试后仍无变化时，任务在 `StuckUiRecovery` 检查点失败，并保留最终快照和尝试次数。坐标列表为空时不执行猜测点击，直接失败。

失败恢复设置持久化在 `%LocalAppData%\BetterBTD\appsettings.json`。除卡死阈值、点击等待时间和恢复坐标顺序外，也可配置连续导航失败阈值和 64 位画面指纹差异容差。坐标会限制在 `0..1919` 和 `0..1079`，列表最多保留 20 项；每次自动任务启动时会从持久化配置创建独立的运行参数快照。
