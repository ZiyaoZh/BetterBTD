...
- `difficultySelect` 与 `easyModeSelect`：难度肖像和模式图标分别绑定 `Easy/Medium/Hard` 与 `Standard/PrimaryOnly/Deflation`；奖励、勋章和本地化说明不参与识别。

其他地图卡、分类按钮、英雄和底部选项虽然可能已有稳定元素 ID 与动作边界，但在拥有自己的检测器前必须保持 `notEvaluated`，控制命令不得点击。中级、困难模式页仍需独立来源证据，不能根据简单模式页或 BetterBTD 内部状态推导。
