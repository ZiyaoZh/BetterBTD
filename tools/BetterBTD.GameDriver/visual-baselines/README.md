...
当前目录覆盖 11 个中文页面、72 个模板和 123 个元素：
...
其他地图卡、分类按钮和底部选项虽然可能已有稳定元素 ID 与动作边界，但在拥有自己的检测器前必须保持 `notEvaluated`，控制命令不得点击。英雄首屏之外的 `Corvus`、`Silas` 仍需要滚动采集；关卡内生命、金币和回合数值当前仅有区域声明，尚未实现独立数值解析。
